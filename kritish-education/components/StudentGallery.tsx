...
export default function StudentGallery() {
  ...
  const handleMouseMove = (e: MouseEvent<HTMLElement>) => {
    ...
    setOpacity(1); // Make it visible when mouse enters
  };

  const handleMouseLeave = () => {
    setOpacity(0); // Hide when mouse leaves
  };
  ...
  return (
    <section 
      ...
      className="relative py-20 bg-slate-200 overflow-hidden"
    >
      
      {/* --- THE MOUSE SPOTLIGHT --- */}
      {/* This overlay moves with your cursor */}
      <div 
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 text-center">
          ...
          <p className="mt-3 text-lg text-gray-500 max-w-2xl mx-auto">
            A mosaic of success stories from students currently thriving in Germany.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 auto-rows-[180px] gap-4 grid-flow-dense">
          ...
          {GALLERY_ITEMS.map((item) => (
            <div 
              key={item.id} 
              className={`group relative overflow-hidden rounded-2xl cursor-pointer transition-all duration-500 hover:shadow-2xl ${item.span}`}
            >
              {/* Image */}
              <img
                src={item.src}
              ...
              {/* Text Info */}
              <div className="absolute bottom-0 left-0 p-5 w-full translate-y-4 group-hover:translate-y-0 transition-transform duration-500 opacity-0 group-hover:opacity-100">
